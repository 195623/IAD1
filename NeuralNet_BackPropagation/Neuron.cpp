...
{
    if( i>=0 && i<this->inputWeights.size() && i<this->lastChange.size() )
    {
        double thisChange = (1-momentum)*value + momentum*lastChange[i] ;

        this->inputWeights[i] += thisChange ;

        this->lastChange[i] = thisChange ;
        //cout << "1-mom = " << 1-momentum << '\n' ;
    }
